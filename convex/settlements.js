    ...
    handler: async(ctx , args) => {
        ...
        if(args.groupId){
            ...
            if(!isMember(args.paidByUserId) || !isMember(args.receivedByUserId)){
                throw new Error("Both parties must be memberss of the group")
            }
        }
    ...
    handler: async (ctx,args) => {
        ...
        if(args.entityType === "user"){
            ...
            const myExpenses = await ctx.db
                .query("expenses")
                .withIndex("by_user_and_group" , (q) => q.eq("paidByUserId" , me._id).eq("groupId", undefined)).collect()

            
            const otherUserExpenses = await ctx.db
                .query("expenses")
                .withIndex("by_user_and_group" , (q) => q.eq("paidByUserId" , other._id).eq("groupId", undefined)).collect()
            ...
            for(const exp of expenses) {
                const involveMe = exp.paidByUserId === me._id || exp.splits.some((s) => s.userId === me._id)
                const involveThem = exp.paidByUserId === other._id || exp.splits.some((s) => s.userId === other._id)

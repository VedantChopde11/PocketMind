...
export const getSimplifiedSettlements = query({
  args: {
    groupId: v.id("groups"),
  },
  ...
  handler: async (ctx, { groupId }) => {
    ...
    settlements.forEach((settlement) => {
      ...
      balances[settlement.paidByUserId] +=
        settlement.amount;

      
      balances[settlement.receivedByUserId] -=
        settlement.amount;
    });
    ...
    await Promise.all(
      group.members.map(async (member) => {
        users[member.userId] = await ctx.db.get(
          member.userId
        );
      })
    );
    ...
    while (
      ...
      settlementList.push({
        from: {
          id: debtors[i].userId,
          name:
            users[debtors[i].userId]?.name ??
            "Unknown",
          imageUrl:
            users[debtors[i].userId]?.imageUrl,
        },

        to: {
          id: creditors[j].userId,
          name:
            users[creditors[j].userId]?.name ??
            "Unknown",
          imageUrl:
            users[creditors[j].userId]?.imageUrl,
        },
